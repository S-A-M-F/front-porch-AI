  ...
  static Future<int?> getKvCacheBytesPerToken(String filePath) async {
    ...
    try {
      ...
      for (var i = 0; i < kvCount; i++) {
        ...
        final key = utf8.decode(bytes.sublist(offset, offset + keyLen.toInt()), allowMalformed: true);
        offset += keyLen.toInt();
        ...
        switch (valType) {
          case 0: // uint8
            ...
            value = data.getUint8(offset); offset += 1; break;
          case 1: // int8
            if (offset + 1 > bytes.length) break;
            value = data.getInt8(offset); offset += 1; break;
          case 2: // uint16
            if (offset + 2 > bytes.length) break;
            value = data.getUint16(offset, Endian.little); offset += 2; break;
          case 3: // int16
            if (offset + 2 > bytes.length) break;
            value = data.getInt16(offset, Endian.little); offset += 2; break;
          case 4: // uint32
            if (offset + 4 > bytes.length) break;
            value = data.getUint32(offset, Endian.little); offset += 4; break;
          case 5: // int32
            if (offset + 4 > bytes.length) break;
            value = data.getInt32(offset, Endian.little); offset += 4; break;
          case 6: // float32
            if (offset + 4 > bytes.length) break;
            value = data.getFloat32(offset, Endian.little); offset += 4; break;
          case 7: // bool
            if (offset + 1 > bytes.length) break;
            value = data.getUint8(offset) != 0; offset += 1; break;
          case 8: // string
            if (offset + 8 > bytes.length) break;
          ...
          case 10: // uint64
            if (offset + 8 > bytes.length) break;
            value = data.getUint64(offset, Endian.little); offset += 8; break;
          case 11: // int64
            if (offset + 8 > bytes.length) break;
            value = data.getInt64(offset, Endian.little); offset += 8; break;
          case 12: // float64
            if (offset + 8 > bytes.length) break;
            value = data.getFloat64(offset, Endian.little); offset += 8; break;
        }
