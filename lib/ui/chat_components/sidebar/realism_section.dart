                ...
                Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 8),
                      NeedsDecayRatesSection(chat: chat),
                      const SizedBox(height: 12),
...
class NeedsDecayRatesSection extends StatefulWidget {
  final ChatService chat;
  const NeedsDecayRatesSection({super.key, required this.chat});

  @override
  State<NeedsDecayRatesSection> createState() => NeedsDecayRatesSectionState();
}

class NeedsDecayRatesSectionState extends State<NeedsDecayRatesSection> {
  bool _expanded = false;

  int _getDecay(String need) {
    if (widget.chat.activeGroup != null) {
      return widget.chat.groupDecayRates[need] ?? 5;
    } else {
      final ext = widget.chat.activeCharacter?.frontPorchExtensions;
      switch (need) {
        case 'hunger': return ext?.needsDecayHunger ?? 5;
        case 'bladder': return ext?.needsDecayBladder ?? 5;
        case 'energy': return ext?.needsDecayEnergy ?? 5;
        case 'social': return ext?.needsDecaySocial ?? 5;
        case 'fun': return ext?.needsDecayFun ?? 5;
        case 'hygiene': return ext?.needsDecayHygiene ?? 5;
        case 'comfort': return ext?.needsDecayComfort ?? 5;
        default: return 5;
      }
    }
  }

  void _setDecay(String need, int value) {
    if (widget.chat.activeGroup != null) {
      widget.chat.setGroupNeedsDecayRate(need, value);
    } else {
      widget.chat.setNeedsDecayRate(need, value);
    }
  }

  Widget _buildDecaySlider(BuildContext context, String label, String key) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Row(
          ...
          children: [
            ...
            Text('${_getDecay(key)}/turn', style: TextStyle(fontSize: 10, color: AppColors.textTertiary(context))),
          ],
        ),
        SizedBox(
          height: 24,
          child: SliderTheme(
            ...
            child: Slider(
              value: _getDecay(key).toDouble(),
              min: 0,
              max: 20,
              divisions: 20,
              activeColor: AppColors.resolve(context, Colors.tealAccent, Colors.teal.shade700),
              inactiveColor: AppColors.borderOf(context).withValues(alpha: 0.3),
              onChanged: (v) => _setDecay(key, v.round()),
            ),
          ),
        ),
      ],
      ...
      children: [
        ...
        if (_expanded) ...[
          ...
          Container(
            ...
            decoration: BoxDecoration(
              color: AppColors.cardOf(context).withValues(alpha: 0.5),
              borderRadius: BorderRadius.circular(8),
              border: Border.all(
            ...
            child: Column(
              ...
              children: [
                Text(
                  'Adjust the baseline per-turn decay rates for each need.',
                  style: TextStyle(fontSize: 10, color: AppColors.textTertiary(context)),
                ),
                const SizedBox(height: 8),
                _buildDecaySlider(context, 'Hunger', 'hunger'),
                _buildDecaySlider(context, 'Bladder', 'bladder'),
                _buildDecaySlider(context, 'Energy', 'energy'),
                _buildDecaySlider(context, 'Social', 'social'),
                _buildDecaySlider(context, 'Fun', 'fun'),
                _buildDecaySlider(context, 'Hygiene', 'hygiene'),
                _buildDecaySlider(context, 'Comfort', 'comfort'),
              ],
            ),
          ),
        ],
